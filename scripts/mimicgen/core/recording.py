...
from .data_utils import as_numpy, quat_wxyz_to_xyzw, to_json_compatible
from .robot_utils import get_arm_eef_pose_robot_frame
...
def _resolve_eef_body_idx(
    ...
    for pattern in candidate_patterns:
        try:
            ...
            body_ids = as_numpy(body_ids, dtype=np.int64).reshape(-1)
            if body_ids.size > 0:
...
class DirectHDF5Recorder:
    ...
    def _slice_env0(value: Any, dtype: np.dtype = np.float32) -> np.ndarray:
        arr = as_numpy(value, dtype=dtype)
        if arr.ndim >= 2:
            arr = arr[0]
    ...
    def _ensure_resizable_dataset(
        ...
        sample_arr = as_numpy(sample, dtype=dtype)
    ...
    def _append_dataset(
        ...
        sample_arr = as_numpy(sample, dtype=dataset.dtype)
        if sample_arr.ndim == 0:
            sample_arr = sample_arr.reshape(1)
    ...
    def _write_fixed_dataset(self, group: Any, name: str, value: Any) -> None:
        arr = np.atleast_1d(as_numpy(value, dtype=np.float32))
    ...
    def _capture_arm_state(
        ...
        if root_pos_w is not None and root_quat_w is not None:
            ...
            root_quat = quat_wxyz_to_xyzw(self._slice_env0(root_quat_w))
    ...
    def _capture_articulation_snapshot(self) -> tuple[dict[str, dict[str, np.ndarray]], np.ndarray | None]:
        ...
        for arm_name in self._state_arm_names:
            ...
            ee_frame_state = None
            if eef_body_idx is not None:
                ee_frame_state = get_arm_eef_pose_robot_frame(
                    arm,
                    eef_body_idx,
    ...
    def _append_sidecar_metadata(self) -> None:
        ...
        episode_rec: dict[str, Any] = {
            "object_initial_pose": to_json_compatible(pose_value),
        }
        scale_value = self._active_episode_meta.get("scale")
        if scale_value is not None:
            episode_rec["scale"] = to_json_compatible(scale_value)
        ...
        for i, articulation in enumerate(self._episode_articulation_states):
            ...
            for _, entity in sorted(articulation.items()):
                ...
                if "joint_velocity" not in entity:
                    continue
                arr = as_numpy(entity["joint_velocity"], dtype=np.float32)
                if arr.ndim == 1 and arr.shape[0] >= 6:
                    single[i] = arr[:6]
                    break
    ...
    def _write_buffered_state_groups(self) -> None:
        ...
        for articulation in self._episode_articulation_states:
            ...
            for arm_name, state in articulation.items():
                ...
                for key, value in state.items():
                    arm_series.setdefault(key, []).append(as_numpy(value, dtype=np.float32))
    ...
    def append_frame(self, frame: dict[str, Any]) -> None:
        ...
        joint_action = as_numpy(frame["action"], dtype=np.float32).reshape(-1)
        joint_pos = as_numpy(frame["observation.state"], dtype=np.float32).reshape(-1)
